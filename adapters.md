...
Este documento explica o conceito de adapters, sua estrutura, como implementá-los,
integrá-los à API e as melhores práticas.

Índice
--------------------------------------------------------------------------------
1. O que é um adapter?
2. Estrutura de dados esperada
   2.1. Nós (nodes)
   2.2. Arestas (edges)
3. Passo a passo para criar um novo adapter
4. Exemplo prático: adapter para banco de dados SQL
...
1. O QUE É UM ADAPTER?
--------------------------------------------------------------------------------
Um adapter é um módulo responsável por extrair dados de uma fonte (sistema de
arquivos, banco de dados, API externa, etc.) e transformá-los em nós e arestas
que o Cytoscape pode renderizar. Cada adapter deve exportar uma função que
retorna uma Promise com a estrutura padronizada.
...
A função do adapter deve retornar um objeto no formato:
    ...
    {
        nodes: [ /* array de objetos de nó */ ],
        edges: [ /* array de objetos de aresta */ ]
    }

2.1. Nós (nodes)
Cada nó deve conter no mínimo os campos `id` e `label`. Campos adicionais são
opcionais, mas podem ser usados para estilização ou informações extras.
...
Campos comuns (usados no estilo):
- `type` (string): define a cor/ícone do nó. Valores suportados no estilo atual:
    'folder', 'html', 'css', 'js', 'pdf', 'docx', 'xlsx', 'pptx', 'external'.
    Se não especificado, assume um estilo padrão.
- `path` (string): caminho completo ou referência (usado em tooltips).
- `size` (number): tamanho do arquivo (opcional).
- `mtime` (number): timestamp da última modificação (opcional).

É recomendável usar a função `normalizeNode` do módulo `../core/schema` para
garantir que os campos essenciais estejam presentes e normalizados.

2.2. Arestas (edges)
Cada aresta deve conter:
- `source` (string): ID do nó de origem.
- `target` (string): ID do nó de destino.
- `relation` (string): tipo de relação. Valores suportados no estilo atual:
    'contains' (hierarquia), 'script', 'style', 'link'. Outros valores podem
    ser adicionados, mas precisam de estilo correspondente no frontend.

Use a função `makeEdge` do módulo `../core/schema` para criar arestas padronizadas.
...
2. Importe as funções auxiliares do schema:
    const { normalizeNode, makeEdge } = require('../core/schema');
3. Implemente a função principal que recebe os parâmetros necessários
   (ex: caminho, URL, credenciais) e retorna os dados.
4. Dentro da função:
   - Use um `Map` para armazenar nós únicos (evita duplicatas).
   - Para cada entidade encontrada, crie um nó com `normalizeNode` e adicione ao Map.
   - Para cada relação, crie uma aresta com `makeEdge` e adicione ao array edges.
    ...
    try {
        ...
        for (const table of tables.rows) {
            ...
            nodesMap.set(tableId, normalizeNode({
                id: tableId,
                label: table.table_name,
                type: 'folder'      // ou um tipo customizado
            }));
            ...
            // Consulta chaves estrangeiras (relacionamentos entre tabelas)
            const fks = await client.query(`
                SELECT
                    kcu.column_name,
                    ccu.table_name AS foreign_table_name
            ...
            for (const fk of fks.rows) {
                const targetId = `table:${fk.foreign_table_name}`;
                if (nodesMap.has(targetId)) {
                    edges.push(makeEdge(tableId, targetId, 'link')); // relação entre tabelas
                }
            }
        }
    } finally {
...
5. INTEGRAÇÃO COM A API
--------------------------------------------------------------------------------
Atualmente o endpoint `/api/graph` chama o adapter do sistema de arquivos. Para
suportar múltiplas fontes, você pode modificar a rota para aceitar um parâmetro
indicando o tipo de fonte. Exemplo:
...
Agora o frontend pode chamar a API com o parâmetro sourceType, por exemplo:
    fetch('/api/graph?path=minha_conexao&sourceType=database')

--------------------------------------------------------------------------------
6. BOAS PRÁTICAS E DICAS
--------------------------------------------------------------------------------
- **Valide os dados**: IDs não podem ser vazios. Arestas devem referenciar nós
  existentes (use `nodesMap.has()` para filtrar).
- **Use Map para evitar duplicatas**: muitas fontes podem gerar o mesmo nó
  várias vezes (ex: mesmo arquivo referenciado de diferentes lugares).
- **Trate erros**: use try/catch na função principal e retorne erros
  significativos (ex: conexão recusada).
- **Documente os tipos de relação** que seu adapter utiliza. Se criar novos
  valores para `relation`, atualize o estilo no frontend para que as arestas
  tenham cores/estilos adequados.
- **Considere performance**: se a fonte tiver muitos dados, implemente
  paginação ou carregamento assíncrono (embora a API atual espere tudo de uma vez).
- **Use caminhos relativos**: nos IDs, prefira usar caminhos relativos ou
  identificadores consistentes para que o frontend possa fazer buscas.
